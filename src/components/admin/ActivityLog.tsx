...
export default function AdminActivityLog({ contactId, initialLogs, initialTotal }: Props) {
    ...
    const loadMore = useCallback(async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/admin/logs/${contactId}?limit=50&offset=${offset}`);
    ...
    const refresh = useCallback(async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/admin/logs/${contactId}?limit=50&offset=0`);
            const data = await res.json();
            setLogs(data.logs);
            setTotal(data.total);
