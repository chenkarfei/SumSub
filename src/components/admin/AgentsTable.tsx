    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch("/api/admin/agents/bulk-delete", {
                ...
                body: JSON.stringify({ ids: agents.map(a => a.id) }),
            });
            const data = await res.json();
            if (!res.ok) { setError(data.error ?? "Failed to remove agents."); return; }
            onSuccess();
        } catch {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                ...
                <h2 className="text-base font-bold text-white mb-1">
                    Remove {agents.length} Agent{agents.length !== 1 ? "s" : ""}
                </h2>
                <p className="text-sm text-slate-400 mb-3">The following agents will be permanently removed:</p>
                <ul className="mb-3 max-h-36 overflow-y-auto space-y-1">
                    {agents.map(a => (
                        <li key={a.id} className="text-sm text-white font-medium truncate">
                            {a.name} <span className="text-slate-400 font-normal">({a.email})</span>
                        </li>
                    ))}
                </ul>
                <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 mb-5">
                    All contacts, verification records, and audit logs for these agents will be permanently deleted. This cannot be undone.
                </p>
...
export default function AgentsTable({ agents }: { agents: AgentRow[] }) {
    ...
    if (agents.length === 0) {
        return <div className="py-12 text-center text-slate-500">No agents yet. Run the migration script to seed the first agent.</div>;
    }
                    ...
                    <tr className="border-b border-slate-200 text-slate-500 text-xs uppercase tracking-wide bg-slate-50">
                        <th className="px-4 py-3 w-10">
                            <input
                                ...
                                checked={allSelected}
                                onChange={toggleAll}
                                className="rounded border-slate-300 bg-white text-purple-600 focus:ring-purple-500/30 cursor-pointer"
                            />
                        </th>
                        <th className="text-left px-4 py-3 font-medium">Agent</th>
                        <th className="text-left px-4 py-3 font-medium hidden md:table-cell">Subdomain</th>
                        <th className="text-left px-4 py-3 font-medium hidden lg:table-cell">Contacts</th>
                ...
                <tbody className="divide-y divide-slate-100">
                    {agents.map(agent => (
                        <tr
                            ...
                            className={`hover:bg-slate-50 transition-colors cursor-pointer ${selected.has(agent.id) ? "bg-red-50" : ""}`}
                        >
                            ...
                            <td className="px-4 py-3 hidden md:table-cell">
                                <span className="font-mono text-xs text-slate-600 bg-slate-100 border border-slate-200 px-2 py-0.5 rounded">{agent.subdomain}</span>
                            </td>
                            <td className="px-4 py-3 hidden lg:table-cell text-slate-600">
                                {agent.totalContacts} total · {agent.verifiedContacts} verified
                            </td>
                            <td className="px-4 py-3 hidden lg:table-cell" onClick={e => e.stopPropagation()}>
                                <ToggleAgentStatusButton agentId={agent.id} isActive={agent.isActive} />
                            </td>
                        </tr>
                    ))}
                </tbody>
