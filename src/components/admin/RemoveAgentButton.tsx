...
export default function RemoveAgentButton({ agentId, agentName }: { agentId: string; agentName: string }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (!res.ok) { setError(data.error ?? "Failed to remove agent."); return; }
    ...
    return (
        <>
            <button onClick={() => setOpen(true)} className="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-medium rounded-lg transition-colors border border-red-200">
                Remove Agent
            </button>
            ...
            {open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                        ...
                        <h2 className="text-base font-bold text-white mb-1">Remove Agent</h2>
                        <p className="text-sm text-slate-400 mb-2">
                            Are you sure you want to remove <span className="text-white font-medium">{agentName}</span>?
                        </p>
                        <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2 mb-5">
                            This will permanently delete the agent and all their contacts, verification records, and audit logs. This cannot be undone.
                        </p>
