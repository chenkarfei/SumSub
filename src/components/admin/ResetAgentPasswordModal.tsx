...
export default function ResetAgentPasswordModal({ agentId, agentName }: { agentId: string; agentName: string }) {
    ...
    return (
        <>
            <button onClick={handleOpen} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-medium rounded-lg transition-colors border border-slate-200">
                Reset Password
            </button>
            ...
            {open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
                        <h2 className="text-base font-bold text-white mb-1">Reset Agent Password</h2>
                        <p className="text-xs text-slate-400 mb-5">Set a new password for <span className="text-white font-medium">{agentName}</span>.</p>

                        {success ? (
                            <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-emerald-400 text-sm mb-4">
                                Password reset successfully. Share the new password with the agent.
                            </div>
                        ) : (
