...
const EMPTY: FormState = { email: "", password: "", name: "" };
            ...
            {open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
                        <h2 className="text-lg font-bold text-white mb-5">Add New Contact</h2>

                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1">Full Name <span className="text-slate-600">(optional)</span></label>
                                <input value={form.name} onChange={set("name")} placeholder="John Smith" className="field-input w-full" />
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-1">Email</label>
                                <input type="email" value={form.email} onChange={set("email")} required placeholder="contact@example.com" className="field-input w-full" />
                            </div>
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-1">Share this with the contact so they can log in.</p>
                            </div>
